...
## Phase 33: PDF結合機能 ★最優先
- [x] 33.1 複数PDFファイルの読み込みUI（ファイル選択ダイアログ）
- [x] 33.2 PDFServiceにmerge機能追加（pdf-lib使用）※既存のsavePDFで対応
- [x] 33.3 結合順序の指定UI（ドラッグ並べ替え）※既存機能で対応
- [x] 33.4 結合PDFのダウンロード機能 ※既存のsavePDFで対応
- [x] 33.5 既存ページへの追加モード対応 ← 「PDFを追加」ボタン実装

## Phase 34: スマホ対応（タッチ操作）
- [x] 34.1 タッチイベントの実装（タップ、ドラッグ、ピンチ）
- [x] 34.2 サイドバーのスワイプ操作対応
- [x] 34.3 注釈のタッチ移動・リサイズ対応
- [x] 34.4 ピンチズーム対応
- [x] 34.5 モバイルUIの最適化（ボタンサイズ、タッチターゲット）

## Phase 35: CMYK変換・印刷用出力
- [x] 35.1 CMYK変換ライブラリの調査・選定
- [x] 35.2 RGB→CMYK変換処理の実装
- [x] 35.3 CMYKプレビュー機能
- [x] 35.4 CMYK PDF出力機能

## Phase 37: スライド部分選択・コピー機能
- [ ] 37.1 範囲選択ツールの実装（矩形選択）
- [ ] 37.2 選択範囲のコピー機能（Ctrl+C）
- [ ] 37.3 選択範囲のペースト機能（同一スライド・別スライド）
- [ ] 37.4 選択範囲のカット機能（Ctrl+X）
- [ ] 37.5 Undo/Redo対応

## Phase 38: 名刺作成・印刷用PDF機能
- [ ] 38.1 名刺サイズテンプレート（91mm x 55mm）
- [ ] 38.2 塗り足し（裁ち落とし）設定（3mm）
- [ ] 38.3 トリムマーク（トンボ）描画機能
- [ ] 38.4 PDFフラット化オプション
- [ ] 38.5 印刷用PDF出力UIの実装

## Phase 39: PWA対応
- [ ] 39.1 Service Workerの設定
- [ ] 39.2 マニフェストファイルの作成

## Phase 40: 自動テストの導入
- [ ] 40.1 テスト戦略の策定・フレームワーク選定 (Vitest vs Playwright)
- [ ] 40.2 テスト環境の構築
- [ ] 40.3 主要ロジックのユニットテスト作成 (Vitest)
- [ ] 40.4 主要フローのE2Eテスト作成 (Playwright)
- [ ] 40.5 CI連携 (GitHub Actions)

## Phase 41: ベクターシェイプ（図形描画）
- [ ] 41.1 図形データモデル設計 (ShapeAnnotation)
- [ ] 41.2 図形描画ツールのUI実装
- [ ] 41.3 各シェイプの実装
    - [ ] 直線
    - [ ] 矢印
    ...
    - [ ] 円・楕円
    - [ ] 自由線 (Pen)
- [ ] 41.4 図形の編集機能 (選択・移動・リサイズ・色変更)
- [ ] 41.5 PDF保存対応

## Phase 42: テキスト検索（In-document Search）
- [ ] 42.1 PDFテキスト抽出機能の実装 (pdf.js)
- [ ] 42.2 検索UIコンポーネントの実装
- [ ] 42.3 検索ロジックと結果リスト表示
- [ ] 42.4 ヒット箇所のハイライト描画
- [ ] 42.5 該当箇所へのページジャンプ・スクロール
